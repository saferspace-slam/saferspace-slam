...
<template>
    <div>
        <form @submit.prevent="form.send"
            class="flex flex-col bg-white sm:px-6 px-4 sm:pt-4 pt-2 sm:pb-6 pb-4 rounded-xl shadow-lg shadow-black/20">
            <div v-if="form.status.value == 'success'" class="text-green-700 flex justify-center">
                <p class="text-green-700 absolute -translate-y-15">Erfolgreich abgeschickt!</p>
            </div>
            <div v-if="form.status.value == 'serverErrror'" class="text-red-700 flex justify-center">
                <p class="text-red-700 absolute -translate-y-15">Fehler beim Senden. Versuche es später nochmal und
                    weise uns gerne per Mail auf
                    diesen Fehler hin.</p>
            </div>
            <div v-if="form.status.value == 'clientError'" class="text-red-700 flex justify-center">
                <p class="text-red-700 absolute -translate-y-15 ">Fehler beim Senden. Überprüfe deine Internetverbindung
